 ...
 */
package com.google.pubsub.kafka.sink.extended.v1;
...
import com.google.pubsub.kafka.common.MetricsRegistry;
import com.google.pubsub.kafka.common.PubSubSinkMetrics;
import com.google.pubsub.kafka.sink.extended.v1.CloudPubSubSinkConnector.OrderingKeySource;
...
public class CloudPubSubSinkTask extends SinkTask {
  ...
  private PubSubSinkMetrics metrics;
  private final String[] customOverrideProperties = {
          ConnectorUtils.MICROMETER_ENABLED,
          ConnectorUtils.STATSD_HOST,
          ConnectorUtils.STATSD_PORT,
          ConnectorUtils.STATSD_STEP,
          ConnectorUtils.DATADOG_SERVICE,
          ConnectorUtils.DATADOG_ENV,
          ConnectorUtils.DATADOG_TAGS
  };
  ...
  public void start(Map<String, String> props) {
    ...
    this.metrics = new PubSubSinkMetrics(registry);
    ...
    metrics.recordStart(() -> {
      ...
      return null;  // because this supplier must return something
    });
  }
  ...
  private ByteString handleValue(Schema schema, Object value, Map<String, String> attributes) {
    ...
    switch (t) {
      ...
      case ARRAY:
        Type arrType = schema.valueSchema().type();
