...
class PollerImpl implements Poller {
  ...
  @VisibleForTesting static final Duration POLL_DURATION = Duration.ofSeconds(10);
    ...
    try {
      ...
      records.forEach(
          consumerRecord -> {
            ...
            boolean keyNullOrEmpty =
                consumerRecord.key() == null || consumerRecord.key().length == 0;
            output.add(
                new SourceRecord(
                    ImmutableMap.of(
                    ...
                    kafkaTopic,
                    // Null partition uses the default kafka partitioner which has key affinity if
                    // the key
